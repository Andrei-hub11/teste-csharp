...
public class AccountController : ControllerBase
{
    ...
    public async Task<IActionResult> Register([FromBody] UserRegisterModel user)
    {
        ...
        try
        {
            ...
            var newIdentityUser = new ApplicationUser { UserName = user.UserName, LastName = user.LastName, Email = user.Email };
    ...
    public async Task<IActionResult> DeleteUserByEmail(string email)
    {
        try
        {
            var result = await _accountService.DeleteUserAsync(email);
